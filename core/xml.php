...
//    var $XMLEncoding = 'ISO-8859-1';
    ...
    /**
     * Sets if the values should be enclosed with CDATA
     *
     * @var bol
     */
    var $XMLEnclosedCDATA = false;
        ...
        foreach ($arr as $key => $value) {
            if (is_numeric($key)) {
                ...
                if (is_array($value)) {
					...
					if($this->XMLEnclosedCDATA)
						$this->XML .= $this->strToXMLSafe("<![CDATA[" .$value. "]]>");
					else
    ...
    function generateXML($xmlHead = true) {
        ...
        $this->XML .= $this->XMLLineBreak;
		$this->XML .= $this->XMLRootOpen;
		$this->XML .= $this->XMLLineBreak;
        $this->XML .= $this->generateTag($this->array);
        $this->XML .= $this->XMLRootClose;
